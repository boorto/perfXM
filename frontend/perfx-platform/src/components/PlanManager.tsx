...
import { Calendar, Play, Plus, Trash2, Edit2, Check, X, Server, FileCode, Terminal, RefreshCw, StopCircle, PauseCircle, Activity, AlertCircle, ArrowLeft, Loader2 } from 'lucide-react';
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';

export const PlanManager: React.FC = () => {
    const { t } = useLanguage();
    const { plans, loading, error, fetchPlans, createPlan, deletePlan, updatePlan, executePlan } = usePlans();
    ...
    const availableScripts = useMemo(() => {
        ...
        return scripts.filter(s => s.project_id === selectedProject.id);
    ...
    const handleStartExecution = async (planId: number) => {
    ...
    const handleStopExecution = async () => {
        ...
        if (selectedPlanId) {
            try {
                ...
                console.error('Update plan status failed:', err);
            }
        }
    };
    ...
    const handleCreateSave = async () => {
        if (formData.name && formData.project_id) {
            try {
                // 获取当前用户ID
                const userStr = localStorage.getItem('perfx_user');
                ...
                alert('创建测试计划失败：' + (err instanceof Error ? err.message : '未知错误'));
            }
        }
    };
                        ...
                        <div className="space-y-6 animate-in slide-in-from-right-8 fade-in duration-300">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={e => setFormData({ ...formData, project_id: parseInt(e.target.value), scriptIds: [] })}
                                    className="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-3 text-white focus:border-blue-500 outline-none transition-colors"
                                >
                                    <option value="0">-- Select Associated Project --</option>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                ...
                                {agents.map(agent => (
                                    <div
                                        ...
                                        className={`p-4 rounded-xl border cursor-pointer transition-all relative overflow-hidden group ${formData.agentIds?.includes(agent.id)
                ...
                <div className="p-6 border-t border-slate-800 bg-slate-900 flex justify-between items-center">
                    ...
                    <div className="flex gap-4 items-center">
                        <button
                            ...
                            disabled={step === 1 && !formData.project_id}
            ...
            <div className="flex-1 bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg relative">
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-800 text-sm">
                        {plans.map(plan => {
                            const projName = projects.find(p => p.id === plan.project_id)?.name || 'Unknown';
                            return (
                                <tr key={plan.id} className="hover:bg-slate-800/30 group transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="text-xs text-slate-400 space-y-0.5">
                                            <div><span className="text-slate-500">Priority:</span> {plan.priority}</div>
                                            <div><span className="text-slate-500">Cases:</span> {plan.total_cases}</div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-slate-400 text-xs font-mono">{new Date(plan.created_at).toLocaleDateString()}</td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${plan.status === 'running' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20 animate-pulse' :
